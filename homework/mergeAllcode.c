...
size_t I, J; // I: trace할때, J: 분기, 점프명령어 파싱할 때 사용

int reg[32] = {0};
int pc = 1000; // 시작주소

int is_error = 0; // 에러 나면 1로 만들고 파일 다시 입력받도록 설정
...
// void print_address(JusoVector *vec); // 얘는 파일에 쓰는걸로 변형해야함
// 파일 출력 함수 추가 선언하기(fileoutput.c 참고)
...
int tokenize(char *str, char *delimiter) {
    int token_count = 0;      // 토큰 개수 초기화
    char temp[10000];         // 원본 문자열 복사본 (충분히 큰 크기)
    strncpy(temp, str, sizeof(temp) - 1);
    temp[sizeof(temp) - 1] = '\0'; // 안전한 널 종료

    char *token = strtok(temp, delimiter);

    while (token != NULL) 
    {
        if (token_count >= 1000) { // 최대 토큰 개수 초과 검사
            return 1000;
        }
        token_count++;       // 토큰 개수 증가
        token = strtok(NULL, delimiter);
    }
    return token_count; // 총 토큰 개수 반환
}
void error_message()
{
    printf("Syntax Error!!\n");
...
void file_read(char *name)
{
    ...
    while (1)
    {
        ...
        char cleaned[10000]; // 공백, 쉼표, 탭 제거된 깔끔한 형태로 저장할 배열
...
void file_write(char *name, char **parsingArray, int parsingArraySize, Juso *array, int jusoArraySize)
{
    char baseFilename[10000]; // 파일 이름을 생성하기 위한 버퍼
...
void handle_I2_type(char *str) // LW, JALR
{
    ...
    int error = separation(tmp); // 괄호 분리 작업
    if (error == 1)
    {
        error_message();
        return;
...
void handle_SB_type(char *str) // 라벨 주소를 가져와야 하므로 일단 int형으로 주소값이 들어온다고 가정
{
    ...
    for (int i = 0; i < label->size; i++)
    {
        ...
        {
            exist = 1;
            immValue = label->data[i].address - juso->data[J].address; // imm값은 타겟 라벨 주소에서 현재 주소를 뺀 값임
        }
    }
...
int main()
{
    ...
    while (1)
    {
        ...
        J = 0;                                  // 분기, 점프명령어 파싱할 때 쓰는 용
        for (size_t i = 0; i < juso->size; i++) // 문자열 파싱 후 배열에 저장
        {
            J = i;
